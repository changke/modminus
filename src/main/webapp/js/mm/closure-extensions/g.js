...
// G-closure may be freely distributed under the MIT license.
// For all details and documentation:
// https://github.com/rhysbrettbowen/G-closure

// Modified by kchang for Web.de Magazine Relaunch 2014 and mail.com Relaunch 2015
...
G = function(input, opt_mod) {
  ...
  if (input.constructor == document.createDocumentFragment().constructor && input.childNodes.length) {
  ...
  if (input.nodeType) {
    input = [input];
  } else if (goog.isString(input)) { // a string is a selector
  ...
  } else if (!input) { // no input is an empty G object
    input = [];
  }
...
GG.cssFilters = {
   ...
   */
  'visible': function(el) {
    return /** @type {boolean} */(G(el).visible());
  },
  /**
   * @param {Element|Node} el to check.
   * @return {boolean} whether element matches the filter.
   ...
   */
  'first': function(el) {
    return el == el.parentNode.firstChild;
  },
  /**
   * @param {Element|Node} el to check.
   * @return {boolean} whether element matches the filter.
   */
  'last': function(el) {
    return el == el.parentNode.lastChild;
  },
  /**
   * @param {Element|Node} el to check.
   * @return {boolean} whether element matches the filter.
...
GG.extend = function(obj, var_args) {
  for (var i = 1; i < arguments.length; i++) {
    goog.object.extend(obj, arguments[i]);
  }
...
G.prototype.visible = function(opt_bool) {
  ...
  return this.each(function(el) {
    goog.style.setElementShown(el, show(el));
  });
};
...
G.prototype.parent = function() {
  var arr = this.map(function(el) {
    return el.parentNode
  }).toArray();
  GG.unique(arr);
  return G(GG.grep(arr, function(el) {
    return el;
  }));
};
...
G.prototype.addClass = function(className) {
  ...
  return this.each(function(el) {
    goog.dom.classlist.add(el, fn(el));
  });
};
...
G.prototype.removeClass = function(className) {
  ...
  var fn = /** @type {Function} */(className);
  return this.each(function(el) {
    goog.dom.classlist.remove(el, fn(el));
  });
};
...
G.prototype.text = function(opt_input) {
  ...
  return this.each(function(el) {
    goog.dom.setTextContent(el, fn(el));
  });
};
